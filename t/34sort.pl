...
sub by_exception {
    print "entering by_exception\n";
    if ( $entering{'by_exception'}++ ) {
	print "restarted by_exception\n";
	return $a <=> $b;
    }
    my $r = DB::restart_at( $nth );
    print "leaving by_exception\n";
    return $a <=> $b;
}
sub seven {
    ...
    my @sorted = sort by_exception 1, 0;
    print "leaving seven\n";
    7
}


for my $nm (qw( two three five seven by_exception )) {
  my $ref = \&$nm;
